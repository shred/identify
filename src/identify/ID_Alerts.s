...
**
* Fetch a description of an alert.
*
*	-> D0.l	Alert ID
*	-> A0.l	^Tags
*	<- D0.l	Error Code
*
		clrfo
ale_DeadStr	fo.l	1	; ^String for Deadend/Recovery
ale_SubsysStr	fo.l	1	; ^String for Subsystem
ale_GeneralStr	fo.l	1	; ^String for General
ale_SpecStr	fo.l	1	; ^String for Specify
ale_StrLength	fo.w	1	; String length (-1 for DBRA)
...
IdAlert		movem.l d1-d7/a0-a3/a5-a6,-(sp)
	...
	;-- gather all strings
		...
		sne	(ale_Localize,a4)
	;-- recovery/deadend?
		tst.l	(ale_DeadStr,a4)	; deadend string wanted?
		beq	.no_dead
		move.l	#MSG_ALERT_RECOVERY,d0
		btst	#31,d7			; deadend alert?
		beq	.deadok
		move.l	#MSG_ALERT_DEADEND,d0
.deadok		move.b	(ale_Localize,a4),d1
		bsr	GetNewLocString		; search
...
.no_dead	tst.l	(ale_SubsysStr,a4)
		...
		lsr	#8,d1
		and	#$7f,d1
		lea	tab_subsystem,a0	; find subsystem
.sub_find	cmp	(subsys_ID,a0),d1	; subsystem ID
		blo	.sub_notfound
		beq	.sub_found
...
.no_subsys	tst.l	(ale_GeneralStr,a4)
		...
		swap	d1
		and	#$7f,d1
		lea	tab_general,a0		; find general cause
...
.no_general	tst.l	(ale_SpecStr,a4)
		beq	.no_spec
		btst	#15,d7			; target object?
		beq	.dest_alert
		move.l	d7,d1			; test target object
		and.l	#$7fff,d1
		lea	tab_objects,a0		; search object
...
.obj_found	move	(object_Spec,a0),d0
		lea	(a5,d0.w),a0		; ^string
		bra	.obj_copy
.dest_alert	move.l	d7,d1			; find target alert
		and.l	#$7fffffff,d1
		lea	tab_alerts,a0		; find alert
		move.l	#MSG_ALERT_UNKNOWN,d0
...
.ale_copy	move.b	(ale_Localize,a4),d1
		bsr	GetNewLocString
.obj_copy	move.l	(ale_SpecStr,a4),a1	; copy
		move	(ale_StrLength,a4),d0
...
.exit		unlk	a4
	...
	;-- error
.err_nolength	moveq	#IDERR_NOLENGTH,d0	; length was zero?
		bra	.exit
...
tab_alerts	alerts	$00000000,	MSG_AN_NOALERT		; $00000000 no Alert
